...
fn main() {
    ...
    info!(log, "slog-term inline error formatting"; "err" => &inline_err);
    info!(log, "slog-term structured error formatting"; "err" => &array_err);

    let json = slog_json::Json::default(io::stdout());
    let log = Logger::root(Mutex::new(json).fuse(), o!());

    info!(log, "slog-json inline error formatting"; "err" => &inline_err);
    info!(log, "slog-json structured error formatting"; "err" => &array_err);
}
